...
- **Functionality**: Add/edit/delete multiple e-wallets (Vodafone Cash, Orange, Etisalat, InstaPay) and bank accounts with customizable daily/monthly limits. Default limits are automatically populated based on wallet type: Mobile wallets (60K daily, 200K monthly), InstaPay/Bank (70K per transaction, 120K daily, 400K monthly). Account names are optional - if left blank, auto-generates mask from mobile number (e.g., vf-012345, or-098765). Mobile numbers must be unique across all wallets to prevent duplicates. Admin can modify default limits from admin panel. Edit wallet balance and limits after creation. Set account status (active, paused, suspended, or issue) to control transaction permissions. Add notes to accounts for tracking reasons for status changes or important information. Optionally override automatic remaining limit calculations with manual values.
- **Purpose**: Centralize all payment methods with intelligent default limits that match Egyptian banking regulations, with unique mobile number validation preventing duplicate accounts, account status management for operational control, and detailed notes for administrative tracking
- **Trigger**: Dashboard "Add Wallet" button, or "Edit" button on wallet card
- **Progression**: Click add → select wallet type → defaults auto-populate → enter mobile number (validated for uniqueness) → optionally enter name (or leave blank for auto-generated mask) → optionally adjust limits → enter balance → set status and notes → save → appears in dashboard list with real-time updates and status badges
- **Success criteria**: All wallets display with correct limits and real-time balance updates, defaults match wallet type, traders can customize limits, admin can change system defaults, data persists between sessions, duplicate mobile numbers are prevented, account name auto-generates if left blank (format: company-code + last 6 digits), wallet editing works correctly with all new fields (status, notes, manual remaining override), status badges display correctly on wallet cards, transactions are blocked for non-active accounts
